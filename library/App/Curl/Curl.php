...
class Curl
{
    ...
    public static $_isCache = false;
    public static $_сacheTimeInterval = null;
    ...
    public function rotateProxyIps()
    {
        ...
        self::$_previousProxyIp = $selectedIp;

        $this->setOpt(CURLOPT_PROXY, $selectedIp);
    }
    ...
    protected function _load($url, $encoding = null, $try = 1)
    {
        ...
        if(self::$_isCache && is_dir(self::$_cacheDir) && is_writable(self::$_cacheDir)) {
            $urlHash = sha1($url);
            $cacheFile = self::$_cacheDir . '/' . $urlHash;
            if (is_file($cacheFile) && !empty($cacheFile) &&
                (self::$_сacheTimeInterval === null || (time()-filemtime($cacheFile)) < self::$_сacheTimeInterval)){
        ...
        if ($stillDo){
            ...
            $urlHash = sha1($url);
            $cacheFile = self::$_cacheDir . '/' . $urlHash;
            if(self::$_isCache && !empty($contents) &&
                is_dir(self::$_cacheDir) &&
                is_writable(self::$_cacheDir) &&
                (!is_file($cacheFile) ||
                    (self::$_сacheTimeInterval !== null && (time()-filemtime($cacheFile)) > self::$_сacheTimeInterval))
            ) {
    ...
    public function bodyDecode($encoding)
    {
        ...
        } else if ($this->_autodetectEncoding) {
