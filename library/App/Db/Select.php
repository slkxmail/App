...
class Select
{
    ...
    public function columns($cols = '*', $correlationName = null)
    {
        ...
        if (!array_key_exists($correlationName, $this->_parts[self::FROM])) {
            /**
             * @see \App\Db\Exception\ErrorException
             */
            throw new \App\Db\Exception\ErrorException("No table has been specified for the FROM clause");
        }
    ...
    {
        if (!is_array($select)) {
            throw new \App\Db\Exception\ErrorException(
        ...
        if (!in_array($type, self::$_unionTypes)) {
            ...
            throw new \App\Db\Exception\ErrorException("Invalid union type '{$type}'");
        }
     ...
     * @param string $part
     * @return mixed
     * @throws \App\Db\Exception\ErrorException(
     */
    public function getPart($part)
    {
        $part = strtolower($part);
        if (!array_key_exists($part, $this->_parts)) {
            throw new \App\Db\Exception\ErrorException("Invalid Select part '$part'");
        }
    ...
    protected function _join($type, $name, $cond, $cols, $schema = null)
    {
        if (!in_array($type, self::$_joinTypes) && $type != self::FROM) {
            ...
            throw new \App\Db\Exception\ErrorException("Invalid join type '$type'");
        }

        if (count($this->_parts[self::UNION])) {
            
            throw new \App\Db\Exception\ErrorException("Invalid use of table with " . self::SQL_UNION);
        }
        ...
        if (!empty($correlationName)) {
            if (array_key_exists($correlationName, $this->_parts[self::FROM])) {
                ...
                throw new \App\Db\Exception\ErrorException("You cannot define a correlation name '$correlationName' more than once");
            }
    ...
    public function _joinUsing($type, $name, $cond, $cols = '*', $schema = null)
    {
        if (empty($this->_parts[self::FROM])) {
            ...
            throw new \App\Db\Exception\ErrorException("You can only perform a joinUsing after specifying a FROM table");
        }
    ...
    {
        if (count($this->_parts[self::UNION])) {
            throw new \App\Db\Exception\ErrorException("Invalid use of where clause with " . self::SQL_UNION);
        }
    ...
    public function _renderLimit($sql, $count, $offset = 0)
    {
        $count = intval($count);
        if ($count <= 0) {
            throw new Exception\ErrorException("LIMIT argument count=$count is not valid");
        }

        $offset = intval($offset);
        if ($offset < 0) {
            throw new Exception\ErrorException("LIMIT argument offset=$offset is not valid");
        }
     ...
     * @return Select
     * @throws \App\Db\Exception\ErrorException If an invalid method is called.
     */
        ...
        if (preg_match('/^join([a-zA-Z]*?)Using$/', $method, $matches)) {
            ...
            if ($type) {
                ...
                if (!in_array($type, self::$_joinTypes)) {
                    ...
                    throw new \App\Db\Exception\ErrorException("Unrecognized method '$method()'");
                }
                if (in_array($type, array(self::CROSS_JOIN, self::NATURAL_JOIN))) {
                    
                    throw new \App\Db\Exception\ErrorException("Cannot perform a joinUsing with method '$method()'");
                }
            } else {
