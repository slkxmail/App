...
use \App\Http\Exception\ErrorException as ErrorException;
...
class Cookie
{
     ...
     * @param string $path
     * @param bool   $secure
     * @throws \App\Http\Exception\ErrorException
     */
    public function __construct($name, $value, $domain, $expires = null, $path = null, $secure = false)
    {
        if (preg_match("/[=,; \t\r\n\013\014]/", $name)) {
            throw new ErrorException("Cookie name cannot contain these characters: =,; \\t\\r\\n\\013\\014 ({$name})");
        }

        if (! $this->name = (string) $name) {
            throw new ErrorException('Cookies must have a name');
        }

        if (! $this->domain = (string) $domain) {
            throw new ErrorException('Cookies must have a domain');
        }
    ...
    public function match($uri, $matchSessionCookies = true, $now = null)
    {
        ...
        if (! ($uri->isValid() && ($uri->getScheme() == 'http' || $uri->getScheme() =='https'))) {
            throw new ErrorException('Passed URI is not a valid HTTP or HTTPS URI');
        }
    ...
    public function toHeaderString()
    {
        ...
        $cookie = array($this->name . '=' . urlencode($value));

        if (substr($cookie[0], -7) == 'deleted') {
            $cookie[] = 'expires=Thu, 01-Jan-1970 00:00:01 GMT';
        } elseif ($this->expires) {
            $cookie[] = 'expires=' . gmdate('D, j-M-Y H:i:s \G\M\T', $this->expires);
        }
    ...
    public static function fromString($cookieStr, $refUri = null, $encodeValue = true)
    {
        // Set default values
        if (is_string($refUri)) {
            $refUri = \Zend\Uri\UriFactory::factory($refUri);
        }
        ...
        if ($refUri instanceof \Zend\Uri\Http) {
        ...
        foreach ($parts as $part) {
            ...
            if (count($keyValue) == 2) {
                ...
                switch (strtolower($k))    {
                    case 'expires':
                        $expires = gmdate('D, j-M-Y H:i:s \G\M\T', strtotime($v));
                        break;
                    case 'path':
                        $path = $v;
                        break;
    ...
    {
        if (! $cookieDomain) {
            throw new ErrorException("\$cookieDomain is expected to be a cookie domain");
        }

        if (! $host) {
            throw new ErrorException("\$host is expected to be a host name");
        }
    ...
    {
        if (! $cookiePath) {
            throw new ErrorException("\$cookiePath is expected to be a cookie path");
        }

        if (! $path) {
            throw new ErrorException("\$path is expected to be a host name");
        }
